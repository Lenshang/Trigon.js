...
let plugEmOut=function(nodes,destination){
	let lastStop;
	for ( let i = 1; i < nodes.length; i++ ) {
		let from;
		let to;
		if ( nodes[i-1].interface === 'custom' ) {
			from = nodes[i-1].output;
		}
		else { // assume native interface
			from = nodes[i-1];
		}
		if ( nodes[i].interface === 'custom' ) {
			to = nodes[i].input;
		}
		else { // assume native interface
			to = nodes[i];
	...
	if ( nodes[nodes.length-1].interface === 'custom') {
		lastStop = nodes[nodes.length-1].output;
	}
	else { // assume native interface
		lastStop = nodes[nodes.length-1];
	}
	lastStop.disconnect(destination);
}
...
let setUpOscillator = function(that, arg){
	...
	//that.soundSource=createOscillator(that.source);
