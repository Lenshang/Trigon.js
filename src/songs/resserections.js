...
var setTrack2=()=>{
    ...
    let piano2 = trigon.triangle({ env: env2, delay: delay2, volume: 0.4 })

    let piano3 = trigon.triangle({ 
...
var setTrack2_2=()=>{
    let _tr=trigon.triangle({ 
        env: {attack: 0.01, decay: 0.005, hold: 0.1, release: 0.5}, 
        volume: 0.5 })
    let _saw=trigon.sawtooth({ 
        filter: { type: 'lowpass', frequency: 9500, q: 1 },
        env: {attack: 0.01, decay: 0.005, hold: 0.1, release: 0.5}, 
        volume: 0.2 })
    let piano = trigon.groupSource([_tr,_saw],{delay: { delayTime: 0.4, wet: 0.25, feedback: 0.25 }})
